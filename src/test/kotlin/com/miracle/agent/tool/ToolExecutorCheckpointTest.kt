...
class ToolExecutorCheckpointTest {
    ...
    @Test
    fun testEditToolRecordsCheckpointWhenUserMessageIdPresent() {
        val file = projectRoot.resolve("src/EditTarget.kt")
        file.parent?.createDirectories()
        file.writeText("before")

        invokeCheckpointCapture(EditTool.getName(), mockTaskState(messageId), file.pathString)
