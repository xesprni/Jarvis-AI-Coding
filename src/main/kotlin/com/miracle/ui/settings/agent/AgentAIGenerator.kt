...
import com.miracle.services.chatCompletion
import com.miracle.services.getSelectedModelId
import com.miracle.services.loadModelConfigs
...
object AgentAIGenerator {
    ...
    suspend fun generate(requirement: String): GeneratedAgent {
        val modelId = try {
            ...
            getSelectedModelId()?.takeIf { models.containsKey(it) }
                ?: throw IllegalStateException("请先在 Jarvis 中配置并选中一个本地模型。")
        } catch (e: Exception) {
            log.warn("Failed to resolve selected model for agent generation", e)
            throw e
        }
