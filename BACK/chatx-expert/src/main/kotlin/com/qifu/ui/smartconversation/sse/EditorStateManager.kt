...
import com.qifu.ui.smartconversation.editor.state.FailedDiffEditorState
import com.qifu.ui.smartconversation.editor.state.RegularEditorState
import com.qifu.ui.smartconversation.editor.state.StandardDiffEditorState
import com.qifu.ui.smartconversation.panels.ResponseEditorPanel
import com.qifu.ui.smartconversation.panels.ResponseEditorPanel.Companion.RESPONSE_EDITOR_STATE_KEY
import okhttp3.sse.EventSource

class EditorStateManager(private val project: Project, private val disposableParent: Disposable) {

    private var currentState: EditorState? = null
    private var diffEditorManager: DiffEditorManager? = null

    fun createFromSegment(
        segment: Segment, readOnly: Boolean = false, eventSource: EventSource? = null, originalSuggestion: String? = null
    ): EditorState {
        val editor = EditorFactory.createEditor(project, segment, disposableParent)
    ...
    fun transitionToFailedDiffState(
        ...
        val newEditor = EditorFactory.createEditor(project, newSegment, disposableParent)
