...
class EditCustomModelDialog(
    ...
    private val modelNameField = JBTextField(originalModel.model).apply {
        ...
        emptyText.text = "例如: gpt-4, claude-3-opus"
    }
    ...
    override fun doValidate(): ValidationInfo? {
        if (modelNameField.text.isNullOrBlank()) {
            return ValidationInfo("请输入模型名称", modelNameField)
        }
        
        // 如果模型名称改变了，检查新名称是否已存在
        if (modelNameField.text.trim() != originalModel.model) {
            if (com.qifu.services.isCustomModelExists(modelNameField.text.trim())) {
                return ValidationInfo("模型 '${modelNameField.text.trim()}' 已存在，请使用其他模型名称", modelNameField)
            }
        }
        
        if (endpointField.text.isNullOrBlank()) {
