...
import com.qifu.ui.settings.mcp.McpMarketChooserDialog
...
class AgentDetailPanel(
    ...
    private fun createToolsSection(): JComponent {
        val header = JPanel(BorderLayout()).apply {
            ...
            add(createToolbarButton("➕从MCP市场添加") { openMcpMarketDialog() }, BorderLayout.EAST)
        }
        return JPanel(BorderLayout()).apply {
            isOpaque = false
            border = JBUI.Borders.emptyBottom(10)
    ...
    private fun openMcpMarketDialog() {
        McpMarketChooserDialog(project) { serverName ->
            refreshToolsAfterMcpInstall(serverName)
        }.show()
    }
