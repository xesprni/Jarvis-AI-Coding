    ...
    suspend fun generateCommitMessage(
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                // 3. 调用 AI 模型生成
                val modelId = loadModelConfigs().values.firstOrNull() { it.alias == ChatxApplicationSettings.settings().defaultJarvisModelId }?.id
                    ?: throw IllegalStateException("未配置默认模型")
                ...
                LOG.error("生成 Commit Message 失败", e)
                return@withContext null
            }
        }
    }
