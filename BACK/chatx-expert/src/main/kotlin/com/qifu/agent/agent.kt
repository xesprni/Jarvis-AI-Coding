...
import com.qifu.agent.AskResponse.ResponseType
import com.qifu.agent.mcp.McpPromptIntegration
...
data class TaskState(
    ...
    companion object {
        private val serviceCache = mutableMapOf<String, Pair<SystemReminderService, FileFreshnessService>>()

        @JvmStatic
        @Synchronized
        fun clearCachedServices(taskId: String) {
            serviceCache.remove(taskId)
        }
    }

    init {
        // 尝试从serviceCache获取已缓存的服务实例
        val cachedServices = serviceCache[taskId]
        systemReminderService = cachedServices?.first ?: SystemReminderService(convId, agentId, project)
        systemReminderService!!.chatMode = chatMode
        systemReminderService!!.isEmbeddedPlanMode = isEmbeddedPlanMode
        fileFreshnessService = cachedServices?.second ?: FileFreshnessService(convId, systemReminderService!!::emitEvent)
        serviceCache[taskId] = systemReminderService!! to fileFreshnessService!!
        try {
            // 创建持久化的shell实例用于执行命令
...
class Task(
    ...
    private val modelId: String = "OPENAI_COMPATIBLE_360-qwen3-coder-480b-a35b",
    ...
    init {
        ...
        if (userInput != null || images != null || files != null) {
            ...
            userInput?.isNotEmpty().let { contents.add(TextContent(userInput)) }
            ...
            if (isMainAgent) {
                convTitle ?: SmartCoroutineScope.get()?.launch {
                    ...
                    TraceUtils.saveAgentConversation(AgentConversation(taskId!!, convTitle ?: "Code Agent Chat (JetBrain Plugin)",
                        Constants.BusinessConstant.SOURCE
                    ))
                }
            }
        }
        ...
        taskState.subTask?.let {
            ...
            if (taskState.waitingAskUserQuestion) {
                ...
                if (response.type == ResponseType.NO) {
                    ChatUtil.updateCodeCompletionLogStatus(response.id, CompletionStatus.REJECTED)
                }
