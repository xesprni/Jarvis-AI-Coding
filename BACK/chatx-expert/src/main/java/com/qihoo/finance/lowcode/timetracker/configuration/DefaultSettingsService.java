 ...
 */
@State(
        name = "CodeTimeTrackerDefaults",
        storages = {@Storage("code-time-tracker-defaults.xml")})
public final class DefaultSettingsService
        implements PersistentStateComponent<TimeTrackerPersistentState> {
    ...
    public static TimeTrackerPersistentState getDefaultState() {
        final DefaultSettingsService service =
                ApplicationManager.getApplication().getService(DefaultSettingsService.class);
        if (service == null) {
