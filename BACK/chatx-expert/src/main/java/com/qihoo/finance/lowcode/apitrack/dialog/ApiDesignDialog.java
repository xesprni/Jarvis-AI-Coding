...
import com.qihoo.finance.lowcode.apitrack.dialog.table.*;
 ...
 */
@Slf4j
public class ApiDesignDialog extends DialogWrapper {
    ...
    public ApiDesignDialog(@Nullable Project project, OperateType operateType) {
        super(project);
    ...
    private void settingComponentsStatus() {
        ...
        this.remarkScroll.setVisible(false);
        this.remarkTextArea.setVisible(false);
    ...
    private void initEditData() {
        ApiNode api = DataContext.getInstance(project).getSelectApiNode();
        InterfaceDetailDTO apiDetail = ApiDesignUtils.apiInterfaceDetail(api);
        if (Objects.isNull(apiDetail)) return;

        // base setting
        this.apiName.setText(apiDetail.getTitle());
        this.apiMethodName.setText(apiDetail.getMethodName());
        this.apiGroup.setSelectedItem(DataContext.getInstance(project).getSelectApiGroupNode());
        this.apiPathUrl.setText(apiDetail.getPath());
        this.apiMethod.setSelectedItem(apiDetail.getMethod().name());
        ...
        if (StringUtils.isNotEmpty(bodyType)) {
            // 定位
            this.requestTabbPanel.setSelectedIndex(0);
            this.bodyTab.setSelectedIndex(bodyTabIndex.getOrDefault(bodyType.toLowerCase(), 0));
            ...
            if (ApiFormType.Json.name().equalsIgnoreCase(bodyType)) {
                EditorComponentUtils.write(project, this.reqBodyJsonTextArea, JSON.formatJson(apiDetail.getReqBody()), false);
                this.reqBodyJsonTableWrap.setStructureJson(apiDetail.getReqBody());

                Object[][] editTableData = this.reqBodyJsonTableWrap.getEditTableData();
                DefaultTableModel model = (DefaultTableModel) this.bodyJsonTable.getModel();
                for (Object[] editTableDatum : editTableData) {
                    model.addRow(editTableDatum);
                }
            }
            if (ApiFormType.File.name().equalsIgnoreCase(bodyType)) {
                EditorComponentUtils.write(project, this.reqBodyFileTextArea, JSON.formatJson(apiDetail.getReqBody()), false);
            }
            if (ApiFormType.Raw.name().equalsIgnoreCase(bodyType)) {
                EditorComponentUtils.write(project, this.reqBodyRawTextArea, JSON.formatJson(apiDetail.getReqBody()), false);
            }
        }
        ...
        if (CollectionUtils.isNotEmpty(reqQuery)) {
            ...
            if (StringUtils.isEmpty(bodyType)) {
                this.requestTabbPanel.setSelectedIndex(Math.min(requestTabbPanel.getTabCount() - 1, 1));
            }
        }
        ...
        if (Objects.nonNull(resBodyType)) {
            this.responseTab.setSelectedIndex(Math.min(responseTabIndex.getOrDefault(resBodyType.toLowerCase(), 0), this.responseTab.getTabCount() - 1));
            if (ApiFormType.Json.name().equalsIgnoreCase(resBodyType)) {
                EditorComponentUtils.write(project, this.resJsonTextArea, JSON.formatJson(apiDetail.getResBody()), false);
                this.resBodyJsonTableWrap.setStructureJson(apiDetail.getResBody());

                Object[][] editTableData = this.resBodyJsonTableWrap.getEditTableData();
                DefaultTableModel model = (DefaultTableModel) this.resJsonTable.getModel();
                for (Object[] editTableDatum : editTableData) {
                    model.addRow(editTableDatum);
                }
            }
        ...
        // remark
        this.remarkTextArea.setText(apiDetail.getTitle());
    }
    ...
    protected void doOKAction() {
        ...
        SwingWorker<Result<?>, Result<?>> swingWorker = new SwingWorker<>() {
            ...
            protected Result<?> doInBackground() {
                ...
                try {
                    ApiDesignDialog.validateSuccess.set(Pair.of(true, StringUtils.EMPTY));
                    result = onOK();
                    success = true;
                } catch (Exception e) {
                    success = false;
                    ...
                    ApiDesignDialog.validateSuccess.remove();
                }
    ...
    private Result<?> onOK() {
        ...
        // 基本设置-methodName
        itfDTO.setMethodName(this.apiMethodName.getText());
        ...
        Boolean validate = ApiDesignDialog.validateSuccess.get().getKey();
        if (!validate) {
            Result<Object> fail = new Result<>();
            fail.setErrorMsg(ApiDesignDialog.validateSuccess.get().getValue());
    ...
    private void initComponentsData() {
        // apiGroup
        DataContext dataContext = DataContext.getInstance(project);
        ApiGroupNode selectApiGroup = dataContext.getSelectApiGroupNode();
        java.util.List<ApiGroupNode> selectApiGroupNodes = dataContext.getSelectApiGroupNodes();
        if (CollectionUtils.isNotEmpty(selectApiGroupNodes)) {
            for (ApiGroupNode apiGroupNode : selectApiGroupNodes) {
                apiGroup.addItem(apiGroupNode);
                if (apiGroupNode.getId().equals(selectApiGroup.getId())) {
                    apiGroup.setSelectedItem(apiGroupNode);
                }
            }
        }
    ...
    private void settingSizes() {
        ...
        apiGroup.setPreferredSize(new Dimension(80, -1));
        remarkTextArea.setPreferredSize(new Dimension(-1, 150));
        ...
        remarkLabel.setFont(new Font("微软雅黑", Font.BOLD, 16));
    ...
    private void initComponents() {
        ...
        ReqUrlPathTableWrap reqUrlPathTable = new ReqUrlPathTableWrap(showEditData(), dialogPane);
        ...
        ReqBodyTableWrap reqBodyFormTableWrap = new ReqBodyTableWrap(showEditData());
        ...
        reqBodyJsonTableWrap = new JsonTableWrap(showEditData());
        ...
        ReqQueryTableWrap reqQueryTable = new ReqQueryTableWrap(showEditData());
        ...
        ReqHeadersTableWrap headersTableWrap = new ReqHeadersTableWrap(showEditData());
        ...
        resBodyJsonTableWrap = new JsonTableWrap(showEditData());
            ...
            //======== scrollPane14 ========
            {
                ...
                //======== panel2 ========
                {
                    ...
                    //======== contentPanel ========
                    {
                        ...
                        // 接口方法名
                        apiMethodNameLabel.setText("        生成Controller方法名");
                        contentPanel.add(apiMethodNameLabel, new GridConstraints(2, 0, 1, 1,
                                GridConstraints.ANCHOR_WEST, GridConstraints.FILL_NONE,
                                GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                                GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                                null, null, null));
                        contentPanel.add(apiMethodName, new GridConstraints(2, 1, 1, 2,
                                GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_BOTH,
                                GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                                GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                                null, null, null));
                        ...
                        contentPanel.add(apiGroupLabel, new GridConstraints(4, 0, 1, 1,
                            ...
                            GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                            null, null, null));
                        contentPanel.add(apiGroup, new GridConstraints(4, 1, 1, 1,
                            GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_BOTH,
                            GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                            GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                            null, null, null));
...
//                                    jsonResponsePanel.add(scrollPane07, new GridConstraints(0, 0, 1, 1,
                        ...
                        paramPanel.add(remarkLabel, new GridConstraints(7, 0, 1, 1,
                            GridConstraints.ANCHOR_WEST, GridConstraints.FILL_NONE,
                            GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                            GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                            null, null, null));
                        ...
                        paramPanel.add(remarkScroll, new GridConstraints(8, 0, 1, 1,
                            GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_BOTH,
                            GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                            GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW,
                            null, null, null));
    ...
    private JBScrollPane remarkScroll;
