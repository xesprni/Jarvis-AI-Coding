...
import com.qihoo.finance.lowcode.convertor.GetInfo;
import com.qihoo.finance.lowcode.convertor.Parameters;
import com.qihoo.finance.lowcode.convertor.complexreturntype.ComplexReturnTypeHandler;
import com.qihoo.finance.lowcode.convertor.complexreturntype.InsertDto;
import com.qihoo.finance.lowcode.convertor.complexreturntype.ListReturnTypeHandler;
import com.qihoo.finance.lowcode.convertor.complexreturntype.MapReturnTypeHandler;
import com.qihoo.finance.lowcode.convertor.complexreturntype.SetReturnTypeHandler;
import com.qihoo.finance.lowcode.convertor.util.PsiClassUtils;
import com.qihoo.finance.lowcode.convertor.util.PsiDocumentUtils;
import com.qihoo.finance.lowcode.convertor.util.PsiToolUtils;
...
import static com.qihoo.finance.lowcode.common.constants.Constants.GenerateSetter.GET;
import static com.qihoo.finance.lowcode.common.constants.Constants.GenerateSetter.IS;
import static com.qihoo.finance.lowcode.common.constants.Constants.GenerateSetter.SET_SETTER_PREFIX;
import static com.qihoo.finance.lowcode.common.constants.Constants.GenerateSetter.STATIC;
import static com.qihoo.finance.lowcode.common.constants.Constants.GenerateSetter.WITH_SETTER_PREFIX;
...
public abstract class GenerateAllSetterBase extends PsiElementBaseIntentionAction {
    ...
    public void invoke(@NotNull Project project, Editor editor,
        ...
        if (psiParent instanceof PsiLocalVariable) {
            ...
            handleWithLocalVariable(psiLocal, project, psiLocal, editor);
        } else if (psiParent instanceof PsiMethod) {
            PsiMethod method = (PsiMethod) psiParent;
            if (method.getReturnType() == null) {
                return;
            }
            handleWithMethod(method, project, method, editor);
        }
    }

    private void handleWithMethod(PsiMethod method, Project project, PsiElement element, Editor editor) {
        ...
        if (dto.getAddedText() != null) {
            ChatUtil.saveCodeCompletionLog(editor, document.getText(), CompletionType.CONVERT_METHOD_BLOCK
                    , CompletionStatus.ACCEPT, dto.getAddedText());
...
//        }
        ...
        ChatUtil.saveCodeCompletionLog(editor, document.getText(), CompletionType.CONVERT_METHOD_BLOCK
                , CompletionStatus.ACCEPT, buildString);
    ...
    public boolean isAvailable(@NotNull Project project, Editor editor,
        ...
        if (generateAllHandler.forBuilder()) {
            ...
            for (PsiMethod method : methods) {
                if (method.getName().equals(Constants.GenerateSetter.BUILDER_METHOD_NAME)&&method.hasModifierProperty(STATIC)) {
                    return true;
                }
            }
            return false;
    ...
    private Boolean isValidAsMethodWithSetterMethod(@NotNull PsiElement element) {
        ...
        if (method.getReturnType() == null || method.getBody() == null) {
            return false;
        }
