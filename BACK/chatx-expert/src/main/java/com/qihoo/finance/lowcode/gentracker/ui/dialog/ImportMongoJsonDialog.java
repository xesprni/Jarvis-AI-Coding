 ...
 */
public class ImportMongoJsonDialog extends DialogWrapper {
    @Getter
    private JPanel mainPanel;
    private EditorImpl editor;
    private final Project project;

    private final MongoJsonTableWrap tableWrap;
    private final JTable jsonTable;

    public ImportMongoJsonDialog(@Nullable Project project, MongoJsonTableWrap tableWrap, JTable jsonTable) {
        super(project);
    ...
    public void initEditor() {
        ...
        this.mainPanel.setPreferredSize(JBUI.size(800, 500));
    }
    ...
    protected void doOKAction() {
        ...
        Map<String, JsonFormNode> jsonNode = new LinkedHashMap<>(MongoJsonTableWrap.INNER_FIELD);
        Map<String, JsonFormNode> importNode = convent(jsonObject);
        // 不允许自定义内置框架字段
        MongoJsonTableWrap.INNER_FIELD.keySet().forEach(importNode::remove);
        jsonNode.putAll(importNode);

        JsonFormNode root = new JsonFormNode();
        root.setType(MongoJsonTableWrap.OBJECT);
    ...
    private Map<String, JsonFormNode> convent(JSONObject jsonObject) {
        ...
        jsonObject.forEach((key, val) -> {
            ...
            if (val instanceof JSONObject) {
                node.setType(MongoJsonTableWrap.OBJECT);
                node.setProperties(convent((JSONObject) val));
            } else if (val instanceof JSONArray) {
                JsonFormNode items = buildArrayItems(val);
                node.setItems(items);
                node.setType(MongoJsonTableWrap.ARRAY);
            } else if (val instanceof String) {
                node.setType(MongoJsonTableWrap.STRING);
            } else if (val instanceof Integer) {
                node.setType(MongoJsonTableWrap.INTEGER);
            } else if (val instanceof Number) {
                node.setType(MongoJsonTableWrap.INTEGER);
            } else if (val instanceof Boolean) {
                node.setType(MongoJsonTableWrap.BOOLEAN);
            } else {
                // 默认String
                node.setType(MongoJsonTableWrap.STRING);
            }
    ...
    private JsonFormNode buildArrayItems(Object val) {
        ...
        items.setName(MongoJsonTableWrap.ITEMS);
