...
import com.qihoo.finance.lowcode.kit.ui.digest.DigestKitDialog;
import com.qihoo.finance.lowcode.kit.ui.json.JsonKitDialog;
 ...
 * @apiNote ToolsPopup
 */
public class KitPopup {
    private static long lastShowTime = 0;
    private JBPopup popup;

    public void show(JComponent locationComponent, Dimension size) {
        ...
        // show popup
        JComponent popupComponent = new KitPopup().createPanel(size);
    ...
    private JComponent createPanel(Dimension maxSize) {
        ...
        {
            shortcutPanel.add(toolButton("JSON格式化", Icons.JSON, e -> {
                closePopup();
                JsonKitDialog.showDialog();
            }));
            shortcutPanel.add(toolButton("编码/加解密", Icons.ENCODE, e -> {
                closePopup();
                DigestKitDialog.showDialog();
            }));
        ...
        // ask mel
        JLabel holder = holderLabel("想拥有更多快捷小工具, 请联系我们 ");
    ...
    @NotNull
    private static JComponent toolButton(String name, Icon icon, ActionListener actionListener) {
        JButton toolButton = new JButton();
        toolButton.setHorizontalAlignment(SwingConstants.CENTER);
        toolButton.setIcon(Icons.scaleToWidth(icon, 30));
