...
public class SQLFileSystem extends VirtualFileSystem implements NonPhysicalFileSystem {

    private static final String PROTOCOL = "mysql";

    public static SQLFileSystem getInstance() {
        return (SQLFileSystem) VirtualFileManager.getInstance().getFileSystem(PROTOCOL);
    }
    ...
    public void openEditor(final SQLVirtualFile SQLVirtualFile) {
        FileEditorManager fileEditorManager = FileEditorManager.getInstance(SQLVirtualFile.getProject());
        fileEditorManager.openFile(SQLVirtualFile, true);
        fileEditorManager.setSelectedEditor(SQLVirtualFile, SQLEditorProvider.ID);
    }
